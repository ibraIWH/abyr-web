...
export default function NewsletterSection() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (email.trim()) {
      // In future: connect to Mailchimp API
  ...
  return (
    <div style={{ background: C.brandRed, padding: "48px 64px", textAlign: "center" }}>
      {submitted ? (
        <>
          <h2 style={{ ...Ser(28, 300, C.cream), marginBottom: 8 }}>Thank You</h2>
          <p style={{ ...F(13, 400, C.cream), opacity: 0.8 }}>You're on the list. We'll keep you posted.</p>
        </>
      ) : (
        <>
          <h2 style={{ ...Ser(28, 300, C.cream), marginBottom: 8 }}>Join the Abyr List</h2>
